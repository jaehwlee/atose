...
csv_df = pd.read_csv(os.path.join(DATAPATH, 'dcase', 'df.csv'), delimiter='\t', names=['file', 'start', 'end', 'path', 'split', 'label'])

binary = np.zeros( (len(csv_df), len(DCASE_TAGS)) ) 

for idx, tag in enumerate(list(csv_df['label'])):
    binary[idx,:] = np.fromstring(tag[1:-1], dtype=np.float32, sep=' ')

df = pd.DataFrame(binary, columns = DCASE_TAGS)

corr_df = df.corr()
corr_df = corr_df.apply(lambda x: round(x, 2))

corr_type = corr_df
tag_type = DCASE_TAGS[5]
...
plt.savefig('dcase_' + tag_type + '_output.png')
