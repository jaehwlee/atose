...
class Split:
    ...
    def get_csv_dataframe(self):
        df_train = pd.read_csv(
            self.make_path("groundtruth_weak_label_training_set.csv"),
            delimiter="\t",
            names=["file", "start", "end", "label"],
        )
        df_test = pd.read_csv(
            self.make_path("groundtruth_weak_label_testing_set.csv"),
            delimiter="\t",
            names=["file", "start", "end", "label"],
        )
        df_eval = pd.read_csv(
            self.make_path("groundtruth_weak_label_evaluation_set.csv"),
            delimiter="\t",
            names=["file", "start", "end", "label"],
        )

        df_train["path"] = [
            os.path.join(self.data_path, "dcase", "npy", "train", "Y" + f[:-3] + "npy")
            for f in df_train["file"]
        ]
        df_test["path"] = [
            os.path.join(self.data_path, "dcase", "npy", "test", "Y" + f[:-3] + "npy")
            for f in df_test["file"]
        ]
        df_eval["path"] = [
            os.path.join(
                self.data_path, "dcase", "npy", "evaluation", "Y" + f[:-3] + "npy"
            )
            for f in df_eval["file"]
        ]
    ...
    def encode(self, label):
        ...
        x[[self.class_to_index[l] for l in label]] = 1.0
        return x
