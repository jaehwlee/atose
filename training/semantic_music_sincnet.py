...
from music_sincnet import resemul


import argparse

parser = argparse.ArgumentParser(description="music-sincnet")
parser.add_argument('--block', type=str, default='rese', choices=['se', 'rese', 'res', 'basic'],
...
# define models
wave_encoder = resemul(block_type=args.block)
...
test_template = "Test Loss : {}, Test AUC : {:.2f}, PR AUC  : {}"
valid_template = "\nEpoch: {}, Valid Loss: {:.5f}, Valid AUC: {:.2f}, PR AUC : {}"
...
def stage1_adam_train_step(wave, labels):
    ...
    with tf.GradientTape() as tape:
        ...
        repre_loss = (1-kld_loss(z2, r1)) + (1 - kld_loss(1-z2, 1-r1))
...
def stage2_train_adam(epochs):
    ...
    if (epoch % 19 == 0 and epoch!= 0):
        ...
        valid_log = valid_template.format(epoch+1, valid_loss.result(), valid_auc.result()*100, pr_auc.result())
        print(valid_log)
...
def stage2_train_sgd(epochs):
    ...
    if (epoch % 19 == 0 and epoch!=0):
        for valid_wave, valid_labels in tqdm(valid_ds):
            stage2_test_step(valid_wave, valid_labels)
        valid_log = valid_template.format(epoch+1, valid_loss.result(), valid_auc.result()*100, pr_auc.result()*100)
        print(valid_log)
...
test_result = test_template.format(valid_loss.result(), valid_auc.result()*100, pr_auc.result()*100)
print(test_result)
...
#tf.keras.models.save_model(classifier, "./models/only_rese/classifier/")
